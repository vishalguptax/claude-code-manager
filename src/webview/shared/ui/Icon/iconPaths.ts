/**
 * Inline SVG geometry for every icon name used across the webview.
 *
 * Source: Lucide (ISC license) — https://lucide.dev. We hand-copy only the
 * icons actually referenced so the bundle never carries the full ~1500-icon
 * set. All icons share the Lucide canvas (24×24 viewBox, stroke-based, round
 * caps/joins) so the <Icon> wrapper can apply uniform attributes. Each value is
 * the inner markup of the <svg> — paths/shapes only, no wrapper element.
 *
 * To add an icon: copy the inner SVG of the matching Lucide glyph here and
 * reference it by name. Keep this list in sync with `grep` of <Icon name=…>.
 */
...
export const ICON_PATHS: Record<string, string> = {
  ...
  // Filters — the funnel is the one that says "narrow a list". The sliders
  // glyph was used here on the argument that the four pickers "adjust" rather
  // than filter, but it is the same shape VS Code and every other toolbar use
  // for preferences, so it read as a settings button sitting next to a search
  // box. The control's job is the label the icon has to carry.
  folder:
  ...
  // Converging chevrons — the two halves fold toward each other.
  "chevrons-down-up": '<path d="m7 20 5-5 5 5"/><path d="m7 4 5 5 5-5"/>',
  // Diverging chevrons — the two halves open away from each other.
  ...
  // Enable / disable a hook. NOT pin-off, which the hook rows used to borrow:
  // a crossed-out pushpin says "unpin", an action hooks do not have. Pause and
  // play are the pair that actually describes stopping and resuming a handler.
  pause:
