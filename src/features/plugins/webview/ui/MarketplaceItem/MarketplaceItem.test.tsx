...
describe("MarketplaceItem", () => {
  ...
  it("explains a pre-registered marketplace that was never fetched", () => {
    render(
      <MarketplaceItem
        marketplace={marketplace({ registered: false, declaredIn: ["project"] })}
      />,
    );
    expect(screen.getByRole("note").textContent).toContain("Pre-registered in project settings");
  });
  ...
  it("explains a marketplace that only a plugin id mentions", () => {
    ...
    expect(screen.getByRole("note").textContent).toContain("never registered");
