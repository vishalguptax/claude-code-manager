You are executing phase F2 feature migration for the account feature.
...
3. src/features/account/ — every file
...
7. src/styles/account.css (empty stub created by F1)

PREREQUISITE: confirm F1 merged.
  git log v2/integration-target --oneline | Select-String "feat\(F1\)"
If empty, STOP and exit.

YOUR BRANCH: v2/feat-account (off v2/integration-target)

FILE ALLOWLIST:
- src/features/account/** (ALL files, create + modify + delete)
- src/styles/account.css
- src/extension/registry.ts — APPEND-ONLY (one line to register account contribution)
...
- docs/releases/v2.0.0-WIP.md — APPEND-ONLY (one bullet under account heading)
...
- .github/**
- src/styles/* except account.css

TASKS:

STEP 1 — Audit existing feature:
- List all current files in src/features/account/
- List all postMessage types this feature sends/receives (grep for message.type)
- List all DOM render entry points in src/features/account/webview/
...
- If no: append the new variant + valibot schema. Commit FIRST as a single isolated commit: `feat(F2/account): add account message schemas`. Push. Then continue.

STEP 3 — Migrate webview to Preact:
- Replace src/features/account/webview/views/*.ts → *.tsx (Preact functional components)
- Replace src/features/account/webview/components/*.ts → *.tsx
- src/features/account/webview/index.tsx — REPLACE F1 stub. Export default Preact component as feature tab content.
...
- No inline styles in TSX. Use className + CSS classes in src/styles/account.css.
...
- src/features/account/messageHandlers.ts (new) — extract dispatch logic from viewProvider.
- src/features/account/viewProvider.ts — slim. Use parseMessage on incoming. Dispatch to messageHandlers.
...
STEP 7 — CSS:
- src/styles/account.css — all feature styles. Use CSS vars from tokens.css. Use VS Code theme vars where possible (var(--vscode-*)).
- No magic numbers — use spacing scale from tokens.

STEP 8 — Cleanup:
- Delete every old vanilla DOM file from src/features/account/webview/. Verify via Glob.
- Run npm run check (Biome). Fix all violations.
- Run npm run typecheck. Fix all errors.
- Run npm test. All tests green.
- Run npm run build. Verify chunks/account-*.js < 50 KB.

STEP 9 — Registry:
- Append one line to src/extension/registry.ts to register account contribution if it provides cross-feature data. Skip if not needed.
...
- feat(F2/account): add account message schemas (if step 2 needed)
- feat(F2/account): preact components for list and detail views
- feat(F2/account): wire signals and useApi hook
- refactor(F2/account): split messageHandlers from viewProvider
- test(F2/account): account component + handler tests
- style(F2/account): move styles to src/styles/account.css
- chore(F2/account): delete vanilla webview files

STEP 12 — Push: git push -u origin v2/feat-account

EXIT CHECKLIST:
- [ ] All old vanilla webview files deleted (Glob src/features/account/webview/**/*.ts returns 0 results, only .tsx)
- [ ] All postMessage validated via valibot on both sides
- [ ] Test coverage ≥ 80% lines
- [ ] No imports from other src/features/ (grep src/features/account/ for "from.*features/" — only own feature allowed)
