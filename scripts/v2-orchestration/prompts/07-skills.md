You are executing phase F2 feature migration for the skills feature.
...
3. src/features/skills/ — every file
...
7. src/styles/skills.css (empty stub created by F1)

PREREQUISITE: confirm F1 merged.
  git log v2/integration-target --oneline | Select-String "feat\(F1\)"
If empty, STOP and exit.

YOUR BRANCH: v2/feat-skills (off v2/integration-target)

FILE ALLOWLIST:
- src/features/skills/** (ALL files, create + modify + delete)
- src/styles/skills.css
- src/extension/registry.ts — APPEND-ONLY (one line to register skills contribution)
...
- docs/releases/v2.0.0-WIP.md — APPEND-ONLY (one bullet under skills heading)
...
- .github/**
- src/styles/* except skills.css

TASKS:

STEP 1 — Audit existing feature:
- List all current files in src/features/skills/
- List all postMessage types this feature sends/receives (grep for message.type)
- List all DOM render entry points in src/features/skills/webview/
...
- If no: append the new variant + valibot schema. Commit FIRST as a single isolated commit: `feat(F2/skills): add skills message schemas`. Push. Then continue.

STEP 3 — Migrate webview to Preact:
- Replace src/features/skills/webview/views/*.ts → *.tsx (Preact functional components)
- Replace src/features/skills/webview/components/*.ts → *.tsx
- src/features/skills/webview/index.tsx — REPLACE F1 stub. Export default Preact component as feature tab content.
...
- No inline styles in TSX. Use className + CSS classes in src/styles/skills.css.
...
- src/features/skills/messageHandlers.ts (new) — extract dispatch logic from viewProvider.
- src/features/skills/viewProvider.ts — slim. Use parseMessage on incoming. Dispatch to messageHandlers.
...
STEP 7 — CSS:
- src/styles/skills.css — all feature styles. Use CSS vars from tokens.css. Use VS Code theme vars where possible (var(--vscode-*)).
- No magic numbers — use spacing scale from tokens.

STEP 8 — Cleanup:
- Delete every old vanilla DOM file from src/features/skills/webview/. Verify via Glob.
- Run npm run check (Biome). Fix all violations.
- Run npm run typecheck. Fix all errors.
- Run npm test. All tests green.
- Run npm run build. Verify chunks/skills-*.js < 50 KB.

STEP 9 — Registry:
- Append one line to src/extension/registry.ts to register skills contribution if it provides cross-feature data. Skip if not needed.
...
- feat(F2/skills): add skills message schemas (if step 2 needed)
- feat(F2/skills): preact components for list and detail views
- feat(F2/skills): wire signals and useApi hook
- refactor(F2/skills): split messageHandlers from viewProvider
- test(F2/skills): skills component + handler tests
- style(F2/skills): move styles to src/styles/skills.css
- chore(F2/skills): delete vanilla webview files

STEP 12 — Push: git push -u origin v2/feat-skills

EXIT CHECKLIST:
- [ ] All old vanilla webview files deleted (Glob src/features/skills/webview/**/*.ts returns 0 results, only .tsx)
- [ ] All postMessage validated via valibot on both sides
- [ ] Test coverage ≥ 80% lines
- [ ] No imports from other src/features/ (grep src/features/skills/ for "from.*features/" — only own feature allowed)
- [ ] Biome + tsc + tests + size all green
- [ ] chunks/skills-*.js ≤ 50 KB
