You are executing phase F2 feature migration for the agents feature.
...
3. src/features/agents/ — every file
...
7. src/styles/agents.css (empty stub created by F1)

PREREQUISITE: confirm F1 merged.
  git log v2/integration-target --oneline | Select-String "feat\(F1\)"
If empty, STOP and exit.

YOUR BRANCH: v2/feat-agents (off v2/integration-target)

FILE ALLOWLIST:
- src/features/agents/** (ALL files, create + modify + delete)
- src/styles/agents.css
- src/extension/registry.ts — APPEND-ONLY (one line to register agents contribution)
...
- docs/releases/v2.0.0-WIP.md — APPEND-ONLY (one bullet under agents heading)
...
- .github/**
- src/styles/* except agents.css

TASKS:

STEP 1 — Audit existing feature:
- List all current files in src/features/agents/
- List all postMessage types this feature sends/receives (grep for message.type)
- List all DOM render entry points in src/features/agents/webview/
...
- If no: append the new variant + valibot schema. Commit FIRST as a single isolated commit: `feat(F2/agents): add agents message schemas`. Push. Then continue.

STEP 3 — Migrate webview to Preact:
- Replace src/features/agents/webview/views/*.ts → *.tsx (Preact functional components)
- Replace src/features/agents/webview/components/*.ts → *.tsx
- src/features/agents/webview/index.tsx — REPLACE F1 stub. Export default Preact component as feature tab content.
...
- No inline styles in TSX. Use className + CSS classes in src/styles/agents.css.
...
- src/features/agents/messageHandlers.ts (new) — extract dispatch logic from viewProvider.
- src/features/agents/viewProvider.ts — slim. Use parseMessage on incoming. Dispatch to messageHandlers.
...
STEP 7 — CSS:
- src/styles/agents.css — all feature styles. Use CSS vars from tokens.css. Use VS Code theme vars where possible (var(--vscode-*)).
- No magic numbers — use spacing scale from tokens.

STEP 8 — Cleanup:
- Delete every old vanilla DOM file from src/features/agents/webview/. Verify via Glob.
- Run npm run check (Biome). Fix all violations.
- Run npm run typecheck. Fix all errors.
- Run npm test. All tests green.
- Run npm run build. Verify chunks/agents-*.js < 50 KB.

STEP 9 — Registry:
- Append one line to src/extension/registry.ts to register agents contribution if it provides cross-feature data. Skip if not needed.
...
- feat(F2/agents): add agents message schemas (if step 2 needed)
- feat(F2/agents): preact components for list and detail views
- feat(F2/agents): wire signals and useApi hook
- refactor(F2/agents): split messageHandlers from viewProvider
- test(F2/agents): agents component + handler tests
- style(F2/agents): move styles to src/styles/agents.css
- chore(F2/agents): delete vanilla webview files

STEP 12 — Push: git push -u origin v2/feat-agents

EXIT CHECKLIST:
- [ ] All old vanilla webview files deleted (Glob src/features/agents/webview/**/*.ts returns 0 results, only .tsx)
- [ ] All postMessage validated via valibot on both sides
- [ ] Test coverage ≥ 80% lines
- [ ] No imports from other src/features/ (grep src/features/agents/ for "from.*features/" — only own feature allowed)
- [ ] Biome + tsc + tests + size all green
- [ ] chunks/agents-*.js ≤ 50 KB
